  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-50">
      <nav className="section-shell pt-5">
        <div
          className={`glass-card neon-border flex items-center justify-between rounded-full px-4 py-3 transition-all duration-500 sm:px-6 ${isScrolled ? "shadow-[0_18px_70px_rgba(2,6,23,0.34)] backdrop-blur-3xl" : "shadow-[0_8px_30px_rgba(2,6,23,0.16)]"
            }`}
        >
            ...
            {links.map((link) => {
              ...
              return (
                <a
                  key={link.href}
                  href={link.href}
                  className={`group relative rounded-full px-4 py-2 text-sm transition ${isActive
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="theme-soft-panel group relative flex h-11 w-[7.2rem] items-center rounded-full p-1 text-white transition hover:border-cyan-300/30 light-mode:text-slate-950"
              aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}
            >
              <motion.span
                layout
                animate={{ x: theme === "dark" ? 0 : 64, rotate: theme === "dark" ? 0 : 180 }}
              ...
              <span className={`${theme === "dark" ? "ml-auto mr-3.5" : "mr-auto ml-3.5"} text-[11px] font-bold uppercase tracking-[0.22em] text-white/95 light-mode:text-slate-600`}>
                {theme}
              </span>
            </button>
